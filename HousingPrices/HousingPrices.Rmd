...
rec = train %>% recipe(sale_price ~ . ) %>% 
  ...
  step_naomit(all_numeric(),-all_outcomes()) %>%  #remove rows that have nas for remaining nas vars
...
```{r}
baked.train %>% miss_var_summary
```

```{r}
baked.train %>% status %>% arrange(-p_na)
```


### 4) create model spec
```{r}
mdl.en.spec = parsnip::linear_reg(
  penalty = tune(),
...
mdl.en.fit %>% predict(new_data = test)
...
1. compare knn impute vs bag impute
