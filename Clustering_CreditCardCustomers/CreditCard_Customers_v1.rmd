...
(a %>% diagnose_outlier() %>% filter(outliers_ratio > 10))
outlier.cols = a %>% diagnose_outlier() %>% filter(outliers_ratio > 10) %>% select(variables) %>% pull()
...
### normality: viz
```{r}
a %>% plot_normality(balance, payments, tenure)
```
...
factoextra::fviz_nbclust(
  ...
  theme_classic()
```
### 2) silhouette analysis with pam and euclidean distancing
```{r cache=TRUE, eval=FALSE}
#observation: cluster::pam is much more computationally expensive than kmeans, takes much, much longer to run
factoextra::fviz_nbclust(
  a.matrix,
  diss = dist(a.matrix, method = "euclidean"),
  ...
  theme_classic()
```
<h4 style="color: blue; font-size:12px;">Makes NO business sense to have only 2 clusters; will go with kmeans # optimal clusters instead of pam's</h4>
...
plot(hclust.a, hang = -1, cex = 0.8, main = NULL, xlab = NULL);abline(h = 180000, col = 'blue', lty = 2)

#At a given height cutoff of x
#Clusters below height x  have a [dist.method] [hclust.method] distance <= height cutoff

hclust.colors = RColorBrewer::brewer.pal(6, 'Paired') #choose # of cols corresponding an optimal number of clusters

hclust.clusters = cutree(hclust.a, 6)
...
<h4 style="color: blue; font-size:12px;">Clusters below a height cutoff of 180000  have a kmeans euclidean distance <= height cutoff</h4>
<h4 style="color: blue; font-size:12px;">Looking at the dendogram, 9 clusters might be 'over-clustering'; will stick with 6 clusters</h4>

# Clustering via Centroid Based Methods

## PAM
```{r fig.width=12, fig.height=9, cache=TRUE, eval=FALSE}
pam = eclust(
  a.matrix,
  FUNcluster="pam", #clustering function
  k=6,
  hc_metric = "euclidean", #metric to be used for calculating (dis)similarities between observations
  hc_method = "ward.D2", #agglomeration method to be used
  )
```
...
library(dbscan)
kNNdistplot(a.matrix, k=10);abline(h=3.5, lty=2, col='blue')
(dbs = dbscan::dbscan(a.matrix, eps = 3.5, minPts = 10))
...
### summary: data
```{r message=FALSE}
a = a %>% mutate(cluster = factor(km$cluster, levels = 1:6))
...
1. There are a lot of outliers, so try a clustering method that does a better job of handling outliers, e.g. dbscan
